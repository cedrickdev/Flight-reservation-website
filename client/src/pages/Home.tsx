...
const GlobeO = dynamic(() => import("@/components/GlobeO").then(module => module.GlobeO), {
  ssr: false,
  loading: () => <span className="hero-globe-placeholder" aria-hidden="true" />,
});
  ...
  return (
    <main id="main-content">
        <section className="hero-section">
          <Image className="hero-image" src={assets.hero} alt="" fill sizes="100vw" quality={82} preload />
          ...
          <div className="hero-content">
            ...
            <h1 aria-label={isFr ? "Votre projet mérite une trajectoire claire." : "Your plan deserves a clear path forward."}>
              {isFr ? <><span className="hero-heading-line">Votre projet mérite</span><br /><em><span className="hero-heading-line hero-heading-accent">une <span className="hero-trajectory-word">traject<span className="hero-globe-letter"><GlobeO label="Globe interactif des pays" /></span>ire</span></span> <span className="hero-heading-line">claire.</span></em></> : <><span className="hero-heading-line">Your plan deserves</span><br /><em><span className="hero-heading-line">a clear path forward.</span></em></>}
            </h1>
        ...
        <section className="wing-section section-shell section-with-visual"><div className="wing-copy"><p className="service-eyebrow">{isFr ? "Le départ se prépare" : "Departure starts here"}</p><h2>{isFr ? <>Prendre de la hauteur,<br /><em>sans perdre le cap.</em></> : <>Take flight,<br /><em>without losing direction.</em></>}</h2><p>{isFr ? "Chaque détail compte avant le départ : le bon document, la bonne réservation, le bon interlocuteur." : "Every detail matters before departure: the right document, the right reservation, the right point of contact."}</p><Link href="/contact" className="button-secondary">{message.quote}<ArrowUpRight size={17} /></Link></div><div className="wing-visual"><Image src={assets.wing} alt={isFr ? "Vue sur l’aile d’un avion au-dessus des nuages" : "View over an airplane wing above the clouds"} width={1200} height={800} sizes="(max-width: 760px) calc(100vw - 40px), 50vw" quality={75} /><span>{isFr ? "En route" : "En route"}</span></div></section>
        ...
        <section className="faq-section">
          <div className="faq-layout">
            ...
            <div className="faq-visual">
              <Image src={assets.faq} alt={isFr ? "Conseillère Trust Elite Travel dans l’agence de Douala" : "Trust Elite Travel adviser in the Douala agency"} width={768} height={1024} sizes="(max-width: 760px) calc(100vw - 20px), 38vw" quality={82} />
            </div>
          </div>
        </section>

        <section className="image-statement section-shell"><div className="image-statement-photo"><picture><Image src={assets.office} alt={isFr ? "Équipe Trust Elite Travel devant les comptoirs de l’agence" : "Trust Elite Travel team in front of the agency counters"} width={1200} height={1600} sizes="(max-width: 760px) calc(100vw - 40px), 50vw" quality={82} /></picture></div><div className="image-statement-copy"><span className="large-number">360</span><p className="service-eyebrow">{isFr ? "Un service, plusieurs points de passage" : "One service, several touchpoints"}</p><h2>{isFr ? <>Préparer.<br /><em>Choisir.</em><br />Partir.</> : <>Prepare.<br /><em>Choose.</em><br />Travel.</>}</h2><p>{isFr ? "Visa, transport, hébergement, assurance, conciergerie ou orientation professionnelle : nous vous aidons à mettre de l’ordre dans ce qui précède le départ." : "Visa, transport, accommodation, insurance, concierge services or career guidance: we help bring order to what comes before departure."}</p><Link href="/contact" className="button-secondary">{message.quote}<ArrowUpRight size={17} /></Link></div></section>

        <section className="contact-banner"><div className="section-shell contact-banner-inner"><Image className="contact-banner-mark" src={assets.logoOnGold} alt="" width={512} height={264} sizes="180px" aria-hidden="true" quality={75} /><div><span className="hero-pretitle"><span className="gold-dot" />{isFr ? "Démarrer votre demande" : "Start your enquiry"}</span><h2>{isFr ? <>Votre prochaine étape<br /><em>commence ici.</em></> : <>Your next step<br /><em>starts here.</em></>}</h2></div><div className="contact-banner-actions"><Link href="/contact" className="button-light">{message.contact}<PlaneTakeoff size={18} /></Link><a href={whatsappHref(contact.phones[0].wa, language)} className="button-light button-light--outline" target="_blank" rel="noopener noreferrer"><WhatsAppBrandIcon size={18} />WhatsApp</a></div></div></section>
    </main>
  );
}
