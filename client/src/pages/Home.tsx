...
import { ArrowDown, ArrowUpRight, BadgeCheck, Check, ChevronDown, Compass, Globe2, MessageCircle, MoveRight, PlaneTakeoff, ShieldCheck, Sparkles, UsersRound } from "lucide-react";
import { Link } from "wouter";
import { GlobeO } from "@/components/GlobeO";
import { SEO } from "@/components/SEO";
import { ServiceCard } from "@/components/ServiceCard";
import { assets, homeMessaging, services, ui } from "@/lib/content";
...
export default function Home() {
  ...
  return (
    <>
      <SEO
        title={isFr ? "Agence de voyage à Douala — billets, séjours et visa" : "Travel agency in Douala — tickets, stays and visa support"}
      ...
      <main>
        <section className="hero-section">
          ...
          <div className="hero-content">
            ...
            <h1>{isFr ? <>Votre projet mérite<br /><em>une traject<span className="hero-globe-letter"><GlobeO label="Globe interactif des pays" /></span>ire claire.</em></> : <>Your plan deserves<br /><em>a clear path forward.</em></>}</h1>
        ...
        <section className="faq-section section-shell"><div className="section-heading"><div><p className="service-eyebrow">{message.faqEyebrow}</p><h2>{message.faqTitle}</h2></div><Sparkles size={24} aria-hidden="true" /></div><div className="faq-list">{message.faq.map(([question, answer]) => <details key={question} className="faq-item"><summary>{question}<ChevronDown size={18} aria-hidden="true" /></summary><p>{answer}</p></details>)}</div></section>

        <section className="image-statement section-shell"><div className="image-statement-photo"><img src={assets.office} alt={isFr ? "Espace de travail Trust Elite Travel avec signalétique et supports de voyage" : "Trust Elite Travel workspace with travel signage and materials"} loading="lazy" width="1122" height="1402" /></div><div className="image-statement-copy"><span className="large-number">360</span><p className="service-eyebrow">{isFr ? "Un service, plusieurs points de passage" : "One service, several touchpoints"}</p><h2>{isFr ? <>Préparer.<br /><em>Choisir.</em><br />Partir.</> : <>Prepare.<br /><em>Choose.</em><br />Travel.</>}</h2><p>{isFr ? "Visa, transport, hébergement, assurance, conciergerie ou orientation professionnelle : nous vous aidons à mettre de l’ordre dans ce qui précède le départ." : "Visa, transport, accommodation, insurance, concierge services or career guidance: we help bring order to what comes before departure."}</p><Link href="/contact" className="button-secondary">{message.quote}<ArrowUpRight size={17} /></Link></div></section>

        <section className="contact-banner"><div className="section-shell contact-banner-inner"><div><span className="hero-pretitle"><span className="gold-dot" />{isFr ? "Démarrer votre demande" : "Start your enquiry"}</span><h2>{isFr ? <>Votre prochaine étape<br /><em>commence ici.</em></> : <>Your next step<br /><em>starts here.</em></>}</h2></div><div className="contact-banner-actions"><Link href="/contact" className="button-light">{message.contact}<PlaneTakeoff size={18} /></Link><a href={`https://wa.me/237655449335`} className="button-light button-light--outline" target="_blank" rel="noreferrer"><MessageCircle size={18} />WhatsApp</a></div></div></section>
      </main>
    </>
  );
}
