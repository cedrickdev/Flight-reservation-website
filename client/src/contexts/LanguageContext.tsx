...
export function LanguageProvider({ children }: { children: React.ReactNode }) { const [language,setLanguage]=useState<Language>(()=>window.localStorage.getItem("trust-elite-language")==="en"?"en":"fr"); useEffect(()=>{document.documentElement.lang=language;window.localStorage.setItem("trust-elite-language",language)},[language]); const value=useMemo(()=>({language,setLanguage,toggleLanguage:()=>setLanguage(current=>current==="fr"?"en":"fr")}),[language]); return <LanguageContext.Provider value={value}>{children}</LanguageContext.Provider> }
