...
import { InstagramBrandIcon, TikTokBrandIcon, WhatsAppBrandIcon } from "@/components/BrandIcons";
import { assets, contact, ui, whatsappHref } from "@/lib/content";
...
function BrandMark({ context, loading, darkSurface = false }: { context: "header" | "footer"; loading?: "eager" | "lazy"; darkSurface?: boolean }) {
  ...
  return <img className={`brand-mark-official brand-mark-official--${context}`} src={source} alt="" width="485" height="512" loading={loading} />;
}
  ...
  return (
    <header className={open ? "site-header menu-is-open" : "site-header"}>
      ...
      <div className="header-inner">
        <Link href="/" className="brand-lockup" aria-label="Trust Elite Travel — accueil"><BrandMark context="header" loading="eager" darkSurface={open} /></Link>
      ...
      {open && <div className="mobile-menu" id="mobile-navigation"><nav aria-label={language === "fr" ? "Navigation mobile" : "Mobile navigation"}>{links.map(link => <Link key={link.href} href={link.href} className={pathname === link.href ? "mobile-nav-link active" : "mobile-nav-link"}>{copy.nav[link.key]}</Link>)}</nav><div className="mobile-menu-footer"><span>{copy.location}</span><a href={whatsappHref(contact.phones[0].wa, language)} target="_blank" rel="noopener noreferrer"><WhatsAppBrandIcon size={16} />WhatsApp <ArrowUpRight size={15} aria-hidden="true" /></a></div></div>}
    </header>
  );
}
  ...
  return (
    <footer className="site-footer">
      <div className="footer-grid">
        ...
        <div className="footer-contact"><span className="footer-label">{language === "fr" ? "Nous écrire" : "Write to us"}</span><a href={`mailto:${contact.email}`}>{contact.email}</a><Link href="/contact" className="footer-link">{copy.request}<ArrowUpRight size={16} aria-hidden="true" /></Link><div className="footer-socials" aria-label={language === "fr" ? "Réseaux sociaux" : "Social networks"}><a className="footer-social-link" href="https://www.instagram.com/trust_elite_travel/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" title="Instagram"><InstagramBrandIcon size={24} /></a><a className="footer-social-link" href="https://www.tiktok.com/@trust.elite.trave" target="_blank" rel="noopener noreferrer" aria-label="TikTok" title="TikTok"><TikTokBrandIcon size={23} /></a></div></div>
      </div>
